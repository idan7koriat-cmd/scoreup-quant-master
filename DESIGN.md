...
- A serif display face (Frank Ruhl Libre) gives headlines real character; the body stays on Heebo, already the strongest choice on hand for long-form Hebrew reading
...
**Body Font:** Heebo (with system-ui, -apple-system, "Segoe UI", sans-serif fallback)

**Character:** A serif with real editorial gravitas paired with a warm, highly legible Hebrew-native sans. The serif signals "this is a serious exam, taken seriously" without tipping into textbook coldness; the sans carries every word the user actually has to read under time pressure, so it stays plain and comfortable rather than characterful.

### Hierarchy
- **Display** (700, `clamp(2rem, 4.5vw, 3.2rem)`, 1.15 line-height): page-level hero headlines only (marketing pages, summary headline). Frank Ruhl Libre.
- **Title** (500, 1.5rem, 1.3 line-height): section headings, card titles, the practice-summary heading. Frank Ruhl Libre.
- **Body** (400, 1rem, 1.75 line-height): every piece of reading content — question text, answer options, explanations, marketing copy. Heebo. This is the highest-exposure role in the product; never substitute the display face here even at large sizes.
...
### Named Rules
**The Reading Content Rule.** Frank Ruhl Libre is a headline face, never a body face. Question text, answer options, and explanations are always Heebo — this is math-dense, long-form reading material, and the serif's characterful strokes cost legibility exactly where the product can least afford it.
